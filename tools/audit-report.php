...
<section class="audit-page" style="padding:48px 16px 96px;background:var(--bg-soft);">
    <div class="tm-container" style="max-width:760px;">
        <div class="audit-no-print" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;flex-wrap:wrap;gap:12px;">
            <div>
                <div style="font-size:0.72rem;font-weight:400;color:var(--accent);text-transform:uppercase;letter-spacing:.06em;">Full Audit Report</div>
                <h1 style="font-size:1.4rem;font-weight:600;color:var(--text);margin-top:4px;"><?= htmlspecialchars($domain) ?></h1>
            </div>
        ...
        <div style="background:#0f172a;border-radius:20px;padding:32px;color:#fff;margin-bottom:28px;">
            <div style="font-size:0.72rem;font-weight:400;color:#4ade80;text-transform:uppercase;letter-spacing:.06em;margin-bottom:10px;"><i class="fa-solid fa-sparkles"></i> AI Executive Summary</div>
        ...
        <div class="audit-print-area" style="background:var(--card);border:1px solid var(--border);border-radius:20px;box-shadow:var(--shadow);padding:36px;">
            <div style="background:var(--accent-soft);border:1px solid var(--accent);border-radius:10px;padding:12px;margin-bottom:24px;">
                <span style="color:var(--accent);font-size:0.85rem;font-weight:500;"><i class="fa-solid fa-circle-check"></i> Score: <?= $results['score'] ?>/100 &middot; <?= count($checks) ?> checks across <?= $results['pages_scanned'] ?> page<?= $results['pages_scanned']==1?'':'s' ?></span>
            </div>
            <?php foreach ($categories as $catName => $catChecks): ?>
            <h3 style="margin:28px 0 14px;text-transform:uppercase;letter-spacing:.04em;font-size:0.8rem;color:var(--accent);font-weight:600;"><?= htmlspecialchars($catName) ?></h3>
            ...
            <div style="display:flex;align-items:flex-start;gap:12px;padding:10px 0;border-bottom:1px solid var(--border);">
                ...
                <div>
                    <div style="font-weight:500;color:var(--text);font-size:0.9rem;"><?= htmlspecialchars($c['label']) ?></div>
                    <div style="color:var(--text-soft);font-size:0.85rem;font-weight:300;"><?= htmlspecialchars($c['detail']) ?></div>
                </div>
            </div>
            <?php endforeach; ?>
            <?php endforeach; ?>
