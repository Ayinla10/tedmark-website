...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['scan_url'])) {
    ...
    } elseif (!auditRateLimit($clientIp, 'scan', 3, 1440)) {
        $error = "You've hit the limit of 3 scans per day. Please try again tomorrow.";
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['request_code']) && !empty($_SESSION['audit_results'])) {
    ...
    } elseif (!auditRateLimit($clientIp, 'otp_request', 3, 1440)) {
        $error = "You've requested too many codes today. Please try again tomorrow.";
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['verify_code']) && !empty($_SESSION['audit_otp_pending'])) {
    ...
    } elseif (!auditRateLimit($clientIp, 'ai_report', 3, 1440)) {
        $error = "You've unlocked too many reports today. Please try again tomorrow.";
    } else {
